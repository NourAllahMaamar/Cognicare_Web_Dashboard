...
export default function SpecialistSupport() {
  ...
  const { authGet, authMutate, authFetch, getUser } = useAuth('specialist');
  ...
  const [files, setFiles] = useState([]); // { file, name, uploading, url, error }
  ...
  const handleFileChange = (e) => {
    ...
    if (total > MAX_FILES) {
      flash(setError, t('support.form.maxFiles', `Max ${MAX_FILES} attachments`));
      return;
    }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      await authMutate('/support', {
        method: 'POST',
        body: { ...form, priority: form.priority || undefined, attachments },
      });
          ...
          <div className="w-full max-w-lg bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            ...
            {selectedTicket.status !== 'resolved' ? (
              <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-surface-dark flex gap-2">
                ...
                />
                <button onClick={() => handleReply(selectedTicket._id)} className="p-2 bg-primary text-white rounded-xl hover:bg-primary-dark transition-all disabled:opacity-50" disabled={!replyText.trim()}>
