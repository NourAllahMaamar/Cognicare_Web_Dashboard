...
function routeToComponent(route = '', t) {
  ...
  if (t) {
    ...
    map[''] = 'Overview'; map['overview'] = 'Overview'; map['organizations'] = 'Organizations';
    map['users'] = 'Users'; map['families'] = 'Families'; map['reviews'] = 'Org Reviews';
    map['training'] = 'Training'; map['caregiver-applications'] = 'Applications';
    map['analytics'] = 'Analytics'; map['system-health'] = 'System Health'; map['settings'] = 'Settings';
  }
...
export default function AdminSystemHealth() {
  ...
  const systemStatus = probeRows.every((r) => r.ok) ? t('adminSystemHealth.allOperational') : t('adminSystemHealth.degraded');
            ...
            {probeRows.map((svc) => (
              <div key={svc.component} className="bg-white dark:bg-surface-dark rounded-xl border border-slate-300 dark:border-slate-800 p-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs font-bold ${svc.ok ? 'text-success' : 'text-error'}`}>{svc.ok ? t('adminSystemHealth.online') : t('adminSystemHealth.error')}</span>
                </div>
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-primary/5">
                      ...
                      <td className="py-3 px-3">{routeToComponent(log.route, t)}</td>
